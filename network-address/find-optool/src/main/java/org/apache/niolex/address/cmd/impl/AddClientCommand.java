 ...
 * @since 2013-12-24
 */
public class AddClientCommand extends BaseCommand {
    ...
    public void processCmd(OPToolService optool, List<String> cmdOps) throws Exception {
        if (cmdOps.size() != 3) {
            error("Usage: addClient <userName> <password>");
            return;
        }
        if (optool.addClient(cmdOps.get(1), cmdOps.get(2))) {
            out("OK");
        } else {
            error("The client account already exists.");
        }
    }
